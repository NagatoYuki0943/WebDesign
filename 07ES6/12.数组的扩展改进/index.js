...
console.log("1. ES6 提供了(...)三个点将一个数组转换为逗号分割来进行处理；...也能用于对象")
let add = (x, y) => x + y;
...
console.log("2. 既然三点运算符通过逗号分割，那么可以想象的应用场景就随意发挥")
...
console.log("1. ES6 提供了 Array.of()方法，它的主要目的是弥补 Array()的不足")
...
console.log("2. ES6 提供了 Array.from()方法，将类似数组的对象或遍历转换成真正的数组")
// PS：对象转换成数组要求比较严格：
// (1) key 必须是数值或字符串数字；
...
console.log("3. ES6 提供了 find() 和 findIndex() 方法，用于查找数组中第一个匹配的值")
// find()查找值,findIndex()查找值得下标
let item = [10, 20, 30, 10, 40];
...
console.log("4. ES6 提供了 fill()方法，可以填充重写数组中的元素值")
item = [1, 2, 3, 4, 5];
...
console.log("5. ES6 提供了 copyWithin 方法，从数组内部复制值，然后粘贴指定位置")
item = [1, 2, 3, 4, 5, 6, 7];
item.copyWithin(2, 0); // 参数1:被替换开始下标;  参数2:替换的值开始下标;  参数3:替换的值结束下标
console.log(item); // Array(7) [ 1, 2, 1, 2, 3, 4, 5 ]

item = [1, 2, 3, 4, 5, 6, 7];
item.copyWithin(2, 0, 2); // 参数1:被替换开始下标;  参数2:替换的值开始下标;  参数3:替换的值结束下标
console.log(item); // Array(7) [ 1, 2, 1, 2, 5, 6, 7 ]
