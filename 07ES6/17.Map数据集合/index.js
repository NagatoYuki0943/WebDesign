...
console.log("1. ES6 提供了 Map 数据集合，是一种以键值对存储的有序列表；没有重复的键值对");
let map1 = new Map();
...
// get() 获取不存在的键值对,返回 undefined
console.log(map1.get('name1')); // undefined
// 不支持通过 [下标] 获取值
...
console.log("2. 我们也可以通过构造函数传递参数的方式进行初始化集合，比如接受一个数组")
let map2 = new Map([
    ['name', 'Mr Wang'],
    ['age', 12]
]);
...
console.log("3. 使用 has()检测、delete()删除、clear()清空等对 Map 集合的操作")
...
// 4. 我们可以使用 forEach 来遍历 Map 集合；
console.log("4. 我们可以使用 forEach 来遍历 Map 集合")
// value:值; key:键值; m:map自身
map1.forEach((value, key, m) => {
    console.log(`${key}-${value} from ${m}`);
});
// name-Yuki from [object Map]
// age-100 from [object Map]
console.log('==================');
...
console.log("5. Map 集合还提供针对对象的 Weak map 集合，添加非对象类型会报错；")
console.log("6. Weak Map 不支持遍历，内部隐藏(无法查看内容)，不支持 foreach 和 size；")
console.log("7. 对于应用场景来说，存放对象的弱引用，不用担心对象被回收后引发的问题；")
