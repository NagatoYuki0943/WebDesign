...
// 除了name都有默认值
console.log("1. ES6 之前函数是无法给参数设置默认值的，而 ES6 支持了这个特性");
function fn1(
    name,
    age = 100,
    arr = [],
...
console.log("2. 函数参数的默认值，也可以是另一个函数的返回值");
function pi() {
    return 3.14;
}
...
console.log("3. 如果只想传递第二往后的参数，参数一保持默认值，可用 undefined 占位");
...
console.log("4. 支持参数二使用参数一的值作为默认值，反之不可以");
...
console.log("5. 解构变量有不定元素，那么函数的参数也可以有不定参数");
// other是以数组的形式存在的
// 不定参数之后不可再有参数
function fn6(name, ...other) {
    console.log(name);
    console.log(other);
...
console.log("6. ES6 提供了一个 name 属性用于获取函数名，以方便开发者");
function fn7() {
